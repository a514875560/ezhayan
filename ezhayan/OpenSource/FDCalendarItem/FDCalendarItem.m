...
@property (nonatomic,assign) CGFloat itenWidth;
- (UILabel *)dayLabel;
...
{
    if (_bgView == nil) {
        _bgView = [[UIView alloc]initWithFrame:CGRectMake((self.bounds.size.width - _itenWidth) / 2, (self.bounds.size.height - _itenWidth) / 2, _itenWidth, _itenWidth)];
...
- (instancetype)initWithCellNum:(NSInteger)num andCellWidth:(CGFloat)width{
    if (self = [super init]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.date]) {
            ...
            cell.bgView.layer.cornerRadius = cell.bgView.frame.size.height / 2;
